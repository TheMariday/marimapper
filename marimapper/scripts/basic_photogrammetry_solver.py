...
def calibrate_camera_ransac(
    obj_points, img_points, n_iterations=500, inlier_threshold=5.0, min_inliers=8
):
    """
    ...
    for _ in range(n_iterations):
        ...
        P = compute_projection_matrix_normalized(
            obj_points[indices], img_points[indices]
        )
    ...
    refined_P = compute_projection_matrix_normalized(
        obj_points[best_inliers], img_points[best_inliers]
    )
...
def triangulate_with_fallbacks(
    idx,
    cameras,
    detections,
    cam_infos,
    ...
    if num_views >= 2:
        ...
        if point_3d is not None:
            ...
            if (
                all_positive_depth
                and max_error < strict_reproj
                and ray_angle > strict_ray_angle
            ):
            ...
            if (
                ...
                and ray_angle > relaxed_ray_angle
            ):
                return TriangulationResult(
                    point=point_3d,
            ...
            if not all_positive_depth:
                ...
                errors_f, depths_f = compute_reprojection_errors(
                    point_flipped, cameras, detections
                )
    ...
    if num_views >= 1:
        ...
        for cam, det in zip(cam_infos, detections):
            ...
            if point_3d is not None:
                ...
                distances_to_known = [
                    np.linalg.norm(point_3d - known_data[k]["pos"])
                    for k in known_data.keys()
                ]
                if (
                    min(distances_to_known) < reference_depth * 2
                ):  # Within 2x expected range
                    return TriangulationResult(
                        point=point_3d,
...
def fill_missing_indices(
    ...
    try:
        ...
        log(
            f"Loaded {len(map_3d)} known 3D points, COM at [{com[0]:.2f}, {com[1]:.2f}, {com[2]:.2f}]"
        )
        log(f"Found {len(views)} 2D view files\n")

        # 2. Calibrate cameras with RANSAC
        log(
            f"Step 1: Camera calibration (RANSAC, {ransac_iterations} iterations, {ransac_threshold}px threshold)"
        )
        log("-" * 80)
        ...
        for view in views:
            ...
            if len(common_indices) < min_calibration_points:
                log(
                    f"  {Path(view['filename']).name}: SKIPPED ({len(common_indices)} pts < {min_calibration_points} required)"
                )
                continue
            ...
            if P is None:
                log(
                    f"  {Path(view['filename']).name}: FAILED (RANSAC found no good model)"
                )
                continue
            ...
            log(
                f"  {timestamp}: {cam_info.inlier_count}/{len(common_indices)} inliers, "
        ...
        if len(calibrated_cameras) < 2:
            log(
                "Error: Need at least 2 calibrated cameras for triangulation.", fg="red"
            )
            return None
        ...
        stats = {"high": 0, "medium": 0, "low": 0, "fallback": 0}
        method_counts = {}
        ...
        for idx in missing_indices:
            ...
            pos_str = (
                f"[{result.point[0]:.4f}, {result.point[1]:.4f}, {result.point[2]:.4f}]"
            )
...
@click.option(
    "--reproj-thresh", default=10.0, help="Max reprojection error for triangulation"
)
@click.option(
    "--min-angle", default=2.0, help="Min ray angle (degrees) for triangulation"
)
